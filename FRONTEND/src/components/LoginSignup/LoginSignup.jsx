...
const IconHouse = (props) => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" {...props}>
    <path d="M3 10.5L12 3l9 7.5V21a1 1 0 0 1-1 1h-5v-6H9v6H4a1 1 0 0 1-1-1v-10.5z" stroke="currentColor" strokeWidth="1.7"/>
  </svg>
);
const IconUser = (props) => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" {...props}>
    <path d="M16 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" stroke="currentColor" strokeWidth="1.7"/>
...
const IconPhone = (props) => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" {...props}>
    <path d="M22 16.92v3a2 2 0 0 1-2.18 2A19.8 19.8 0 0 1 12 19a19.5 19.5 0 0 1-7.82-7.82A19.8 19.8 0 0 1 2.08 4.18 2 2 0 0 1 4.06 2h3a2 2 0 0 1 2 1.72c.12.9.33 1.77.63 2.6a2 2 0 0 1-.45 2.11L8.09 9.91A16 16 0 0 0 14.09 15l1.48-1.15a2 2 0 0 1 2.11-.45c.83.3 1.7.51 2.6.63A2 2 0 0 1 22 16.92z" stroke="currentColor" strokeWidth="1.7"/>
  </svg>
);
const IconPin = (props) => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" {...props}>
    <path d="M12 22s7-4.35 7-11a7 7 0 0 0-14 0c0 6.65 7 11 7 11z" stroke="currentColor" strokeWidth="1.7"/>
...
const IconId = (props) => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" {...props}>
    ...
    <path d="M14 9h5M14 13h5" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round"/>
  </svg>
);
  ...
  async function handleSubmit() {
    try {
      ...
      if (mode === "login") {
        ...
        const payload = { username: identifier.trim(), password: password.trim() };
        ...
        localStorage.setItem("usuario", JSON.stringify(data?.values?.usuario || {}));
      ...
      // SIGNUP
      if (!username.trim() || !nombre.trim() || !correo.trim() || !password.trim()) {
      ...
      if (tipo === "cliente") {
        ...
        const { res, data } = await postJSON(`${BASE}/register/`, payload);
        if (!res.ok) throw new Error(data?.message || `HTTP ${res.status}`);
        ...
        const { res, data } = await postJSON(`${BASE}/registerAgente/`, payload);
        if (!res.ok) throw new Error(data?.message || `HTTP ${res.status}`);
          ...
          >
            <button
              ...
              style={{
                ...
                padding: "8px 12px",
                cursor: "pointer",
                borderBottom: mode === "login" ? `2px solid ${palette.brand}` : "2px solid transparent",
              }}
            >
              Iniciar sesión
            </button>
            <button
              ...
              style={{
                ...
                padding: "8px 12px",
                cursor: "pointer",
                borderBottom: mode === "signup" ? `2px solid ${palette.brand}` : "2px solid transparent",
              }}
            >
              Registrarse
            </button>
          ...
          <div style={{ display: "grid", gap: 12 }}>
            {mode === "login" ? (
              ...
              <>
                ...
                <Field icon={<IconId />} placeholder="Usuario" value={username} onChange={setUsername} />
                <Field icon={<IconUser />} placeholder="Nombre completo" value={nombre} onChange={setNombre} />
                <Field icon={<IconMail />} placeholder="Correo electrónico" type="email" value={correo} onChange={setCorreo} />
                <Field icon={<IconPhone />} placeholder="Teléfono (opcional)" value={telefono} onChange={setTelefono} />
                {tipo === "cliente" && (
                  <Field icon={<IconPin />} placeholder="Ubicación (opcional)" value={ubicacion} onChange={setUbicacion} />
                )}
                {tipo === "agente" && (
                  <>
                    <Field icon={<IconId />} placeholder="Número de licencia" value={numeroLicencia} onChange={setNumeroLicencia} />
                    <Field icon={<IconId />} placeholder="Años de experiencia" type="number" value={experiencia} onChange={setExperiencia} />
                  </>
                )}
                <Field
                  icon={<IconLock />}
                  ...
                  onChange={setPassword}
                  right={
                    <button type="button" onClick={() => setShowPwd((v) => !v)} style={ghostBtnStyle}>
                      {showPwd ? "🙈" : "👁️"}
                    </button>
                  }
                />
            ...
            {msg && (
              <div
                style={{
                  fontSize: 13,
                  color: msg.toLowerCase().includes("error") || msg.toLowerCase().includes("http") ? palette.danger : palette.dim,
            ...
            {mode === "login" && (
              <div style={{ fontSize: 13, color: palette.dim }}>
                ¿Olvidaste tu contraseña? <span style={{ color: palette.brand, cursor: "pointer" }}>Recupérala</span>
              </div>
            )}
          </div>
        </div>
